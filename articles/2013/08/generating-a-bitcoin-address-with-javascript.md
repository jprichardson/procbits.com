...
I'd be remiss if I didn't mention anything about random number generation. Random number generation is the basis of most cryptography and Bitcoin. Your Bitcoin addresses are only as secure as your random number generator. A random number generator that is said to be cryptographically secure is good enough to use for cryptography in that there is enough [entropy](http://en.wikipedia.org/wiki/Entropy) for a person to predict the number generator. `Math.random()` is not cryptographically secure. This is because `Math.random()` is predictable. If it's predictable, an attacker could figure out your private key from your public key. The implications of someone else knowing your private key means that they can also spend your Bitcoins.
